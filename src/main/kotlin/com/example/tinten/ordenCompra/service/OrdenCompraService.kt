...
class OrdenCompraService(
    ...
    fun create(request: OrdenCompraCreateRequest): OrdenCompraDto {
        ...
        request.productos.forEach { productoReq ->
            val producto = productoRepository.findByNombre(productoReq.productName.trim())
                .orElseThrow { NotFoundException("Producto no encontrado: ${productoReq.productName}") }
        ...
        request.productos?.let { nuevosProductos ->
            ...
            ordenCompra.productos.addAll(
                nuevosProductos.map { prodReq ->
                    val producto = productoRepository.findByNombre(prodReq.productName.trim())
                        .orElseThrow { NotFoundException("Producto no encontrado: ${prodReq.productName}") }
    ...
    fun validarProductos(productos: List<ProductoOrdenCompraCreateDto>) {
        val names = productos.map { it -> it.productName.trim().lowercase() }
        ...
        productos.forEach {
            if (it.cantidad <= 0) throw ValidationException("La cantidad de '${it.productName}' debe ser > 0")
            if (it.precioUnitario <= java.math.BigDecimal.ZERO)
                throw ValidationException("El precio unitario de '${it.productName}' debe ser > 0")
        }
    }
}